...
const PieChart = ({ data }: { data: { name: string, value: number, color: string }[] }) => {
  ...
  if (total === 0 || data.length === 0) {
    return (
      ...
      >
        <p className="m-0 text-muted" style={{ fontSize: '0.9rem' }}>Sin Datos</p>
      </div>
    );
  }
  ...
  return (
    ...
    <div className="d-flex justify-content-center align-items-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox="0 0 100 100">
        ...
        <text x="50" y="50" textAnchor="middle" fontSize="10" fontWeight="bold" fill="#343a40">
          {total}
        </text>
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        setError("No se pudieron cargar los datos del dashboard. Inténtalo de nuevo.");
      } finally {
        setLoading(false);
      }
    }
        ...
        <div className="bg-white rounded-4 shadow-sm p-4">
          ...
          <div className="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center mb-4 gap-3">
            ...
            <div className="d-flex flex-wrap gap-2">
              ...
              <button className="btn btn-sm btn-outline-secondary">Última semana</button>
              <button className="btn btn-sm btn-outline-secondary">Último mes</button>
              <button className="btn btn-sm btn-outline-secondary">Últimos 6 meses</button>
              <button className="btn btn-sm btn-outline-secondary">Último año</button>
              <button className="btn btn-sm btn-outline-secondary">Todo el tiempo</button>
            </div>
          </div>
          <div className="mb-4">
            ...
            <div className="d-flex flex-wrap gap-2">
              ...
              <button className="btn btn-sm btn-outline-secondary">Bache</button>
              <button className="btn btn-sm btn-outline-secondary">Alumbrado</button>
            </div>
          </div>
          ...
          <div className="row g-4">
            ...
            <div className="col-12 col-lg-8">
              <div className="card shadow-sm border-0">
                <div className="card-body">
                  <h5 className="card-title fw-semibold">Reportes por estado</h5>
                  ...
                  <div className="d-flex flex-column flex-md-row align-items-center justify-content-around mt-4 gap-4">
                    ...
                    <div className="d-flex flex-column gap-3 mb-4 mb-md-0" style={{ minWidth: '160px' }}>
                      
                      {/* En Revisión y Proceso */}
                      <div className="d-flex flex-column">
                        <p className="m-0 fw-semibold text-warning" style={{ fontSize: '0.9rem' }}>En revisión / Proceso</p>
                        <p className="m-0 text-muted" style={{ fontSize: '0.8rem' }}>{inProcessTotal} totales</p>
                      </div>

                      {/* Aprobados */}
                      <div className="d-flex flex-column">
                        <p className="m-0 fw-semibold text-success" style={{ fontSize: '0.9rem' }}>Aprobados</p>
                        <p className="m-0 text-muted" style={{ fontSize: '0.8rem' }}>{totalApproved} totales</p>
                      </div>

                      {/* Completados y Cierre */}
                      <div className="d-flex flex-column">
                        <p className="m-0 fw-semibold text-primary" style={{ fontSize: '0.9rem' }}>Completados / Cierre</p>
                        <p className="m-0 text-muted" style={{ fontSize: '0.8rem' }}>{completedTotal} totales</p>
                      </div>

                      {/* No Aprobados */}
                      <div className="d-flex flex-column">
                        <p className="m-0 fw-semibold text-danger" style={{ fontSize: '0.9rem' }}>No Aprobados</p>
                        <p className="m-0 text-muted" style={{ fontSize: '0.8rem' }}>{notApprovedCount} totales</p>
                      </div>
                    </div>
                    ...
                    <div className="d-flex align-items-center gap-4 flex-wrap justify-content-center">
                      ...
                      {/* Leyenda del gráfico */}
                      <ul className="list-unstyled m-0" style={{ fontSize: '0.85rem' }}>
            ...
            <div className="col-12 col-lg-4">
              <div className="card h-100 shadow-sm border-0">
                <div className="card-body d-flex flex-column justify-content-center">
                  ...
                  <div className="mt-3">
                    <p className="m-0 fs-5 fw-bold text-dark">{total_reports} reportes totales</p>
            ...
            <div className="col-12">
              <div className="card shadow-sm border-0">
                <div className="card-body">
                  <h5 className="card-title fw-semibold mb-3">Colonias con mayor número de reportes</h5>
                  
                  <ol className="list-unstyled m-0"> 
                    {topNeighborhoods.map((item, index) => (
                      <li key={index} className="mb-1" style={{ fontSize: '0.95rem' }}>
                        <span className="fw-bold me-2">{index + 1}.</span>
                        {item.neighborhood}: {item.reports_counted} reportes
                      </li>
                    ))}
                    {topNeighborhoods.length === 0 && (
                      <p className="text-muted">No hay datos de conteo de reportes por colonia.</p>
                    )}
                  </ol>
                </div>
              </div>
